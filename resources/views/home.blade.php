            ...
            <div class="col-md-10">
                ...
                <div class="card" style="background: transparent;">
                    ...
                    @forelse($articles->reverse()  as $article)
                        <div class="card mt-10 mb-10" style="margin-bottom: 30px;">
                            ...
                            <div class="card-body">
                                ...
                                <h6 class="card-title text-muted bg-light">Author: {{ \App\Models\User::find($article->user_id)->name }}</h6>
                            ...
                            @if(Auth::user()!=null)
                                @if(Auth::user()->can('create', $article))
                                    <a href="{{ route('article.edit', [$article->id]) }}" title="Edit" class="btn btn-sm btn-light">Edit</a>
                                        <a href="javascript:void(0)" onclick="deleteArticle({{ $article->id }})" class="btn btn-sm btn-dark">Delete</a>
                                @endif
                            @endif
                        </div>
                    @empty
